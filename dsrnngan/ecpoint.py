...
breakpoints = np.loadtxt("/ppdata/ecPoint/CompFiles/MapFunc/Dev/ecPoint_Rainfall/012/Vers1.0.0/BreakPointsWT.txt",
...
def getALLdata(date,hour,ifs_fields=['cp','tp','u700','v700','cape','cdir'],
               tp_axis = 1):
    #Load data 
    inp,nim=load_ifs_nimrod_batch([date],ifs_fields=ifs_fields,hour=hour,norm=False,
...
def classifyALLerror(date,hour,bpoints=breakpoints_hourly,
    ...
    for i in range(bpoints.shape[0]):
        pointset = (bins == i)
        if (pointset.sum() > 0):
            era_prec = np.expand_dims(filtered_era[bins==i,1],-1)
            errors = (nimrod_eragrid_nonzero[bins==i,:] - \
                           era_prec)/era_prec
...
def saveclass(dates):
    for i,d in enumerate(dates):
        ...
        for j,dta in enumerate(ans):
            if len(dta) > 0:
                merged = np.array(dta)
                np.savetxt(f"/ppdata/tmp/{year}/Fer{j}_{i}.txt",merged)
    return
...
def loadgroup(idx,year=[2018]):
    ...
    for yr in year:
        for i in range(400):
            flename=f"/ppdata/tmp/{yr}/Fer{idx}_{i}.txt"
            if i == 0:
                print(flename)
...
def classifyBin(date,bpoints=breakpoints_hourly,era_fields=['prc','pr','u700','v700','cape','cdir']):
    ...
    for binnum in range(bpoints.shape[0]):
        hist,bins_edges=np.histogram((nimrod_eragrid_nonzero[bins==binnum]-
                                     filtered_era[bins==binnum,1])/filtered_era[bins==binnum,1],
...
def classifyerror(date,bpoints=breakpoints_hourly,era_fields=['prc','pr','u700','v700','cape','cdir'],
    ...
    for i in range(bpoints.shape[0]):
        pointset = (bins == i)
        if (pointset.sum() > 0):
            errors = list((nimrod_eragrid_nonzero[bins==i] - \
                           filtered_era[bins==i,1])/filtered_era[bins==i,1])
            priordata[i]+=errors
...
def crps(data_generator,ecpointcdf,log=False):
    ...
    for i in range(data_generator.__len__()):
        ...
        pred=ecpoint.predictupscalecdf(dta[0]['generator_input'],cdf=ecpointcdf,logout=log)
        if log:
