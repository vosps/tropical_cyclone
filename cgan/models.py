...
def generator(mode,
            #   input_channels=9,
              input_channels=1, # I think this should be 1 as i only have 1 feature right now
              latent_variables=1,
              noise_channels=8,
              filters_gen=64,
              img_shape=(100, 100),
              constant_fields=2,
            #   constant_fields=0, # maybe this will fix everything?
              conv_size=(3, 3),
              stride=1,
              relu_alpha=0.2,
              norm=None,
              dropout_rate=None):
    """
    This function basically sets up your generator model. It initialises all of the shapes that would input to the model
    if you don'y want the constant variables such as land sea mask and orography, then i think you need to 
    change the shapes of things in here...
    """
    # Network inputs
    ...
    # remove constant fields for now
    # constant fields
    ...
    if mode in ('det', 'VAEGAN'):
        ...
        # Concatenate all inputs together
        # generator_output = concatenate([generator_input, upscaled_const_input, noise_input]) # don't need const for now
