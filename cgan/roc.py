...
def plot_roc_curves(*,
    ...
    precip_values = np.array([0.01, 0.5, 5, 10, 50])
    ...
    if predict_full_image:
        ...
        data_predict = DataGeneratorFull(dates=dates,
                                         ...
                                         crop=True,
                                         shuffle=False,
                                        #  constants=True, # constants=True gives an error as I don't have the constants netcdf file 
                                         constants=None,
                                         hour=0,
                                         ifs_norm=True,
    ...
    for model_number in model_numbers:
        ...
        if not os.path.isfile(gen_weights_file):
            ...
            # model.gen.load_weights('logs/models-disc_weights.h5')
            ...
            # inputs, outputs = data_predict
            
            print('predict full image: ',predict_full_image)
            ...
            for i in range(num_images):
                ...
                if predict_full_image:
                    ...
                    im_real = data.denormalise(outputs)[...,0]         
                if mode == 'det':
                    pred_ensemble = []
                    ...
                    for j in range(ensemble_members):
                        print('j',j)
                        # inputs['noise_input'] = noise_gen() #don't think we need this but dunno
                        if mode == 'GAN':
                            # pred_ensemble.append(data.denormalise(model.gen.predict([inputs['generator_input'], 
                            #                                                          inputs['constants'], 
                            #                                                          inputs['noise_input']]))[...,0])
                            pred_ensemble.append(data.denormalise(model.gen.predict([inputs,noise_gen()]))[...,0])
                        elif mode == 'VAEGAN':
    ...
    plot_ecpoint = False # if true raises an error
    if plot_ecpoint:
        dates=get_dates(predict_year)
        if predict_full_image:
            batch_size = 4
