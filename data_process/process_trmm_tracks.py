...
def handle_regrid(fp):
	if Path(fp[:-4] + '_regrid.HDF').is_file():
		print(fp[:-4] + '_regrid.HDF')
		regrid_fp = fp[:-4] + '_regrid.HDF'				
...
tracks = tracks[tracks['SEASON'] >= 1998]
tracks = tracks[tracks['SEASON'] <= 2019]
...
def generate_days(year):
	...
	if str(year) == '1997':
		days = [500]
	# elif year == 2000:
	# 	days = ["{0:03}".format(i) for i in range(153,202)]
	# elif year == 2001:
	# 	days = [500]
	# elif year == 2020:
...
for track_i in range(len(tracks)):
	...
	track_hour = "{:02d}".format(int(hour))
	# if '-' in track_hour:
	# 	track_hour = "235959"
	# 	doy = str(int(doy) - 1)
	# print(doy)
	# print(doy[4:])
	# print(year)
	# print(str(print(year)))
	# print(generate_days(year))
	...
	for f in rainfall_fps:
		if (track_hour in f) and f.endswith('HDF'):
